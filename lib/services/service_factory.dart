...
import 'graduation/mock_graduation_service.dart';
...
/// - 一般帳號 → 回傳真實實作
///
/// 成績 / 畢業門檻 / 課表 / 請假的真實實作**就是 scraper 本身** —— 它們直接實作
/// 對應的 Service 介面，中間不再有一層純轉呼叫的類別。認證與行事曆另有真實邏輯
/// （錯誤轉譯、合併端點），因此保留各自的實作類別。
class ServiceFactory {
  ...
  late final GraduationScraper _graduationScraper = GraduationScraper(
    client.dio,
  );
  ...
  CourseService get courseService =>
      isDemoMode ? _mockCourse : _scheduleScraper;
  AbsentService get absentService => isDemoMode ? _mockAbsent : _absentScraper;
